<script lang="ts">
	...
	type LoginAdmin = { id: number; name: string };

	let pin = $state('');
	let error = $state<string | null>(null);
	let loading = $state(false);
	let admins = $state<LoginAdmin[]>([]);
	...
	async function submit() {
		error = null;
		if (admins.length > 1 && userId == null) {
			error = 'Выбери администратора';
			return;
		}
		if (pin.length < 4) {
		...
		const res = await fetch('/api/admin/login', {
			...
			body: JSON.stringify({ pin, userId: userId ?? undefined })
		});
		loading = false;
		if (!res.ok) {
			pin = '';
...
<main class="flex min-h-dvh flex-col items-center justify-center px-6 pb-8">
	<div class="w-full max-w-sm">
		...
		{#if admins.length > 1}
			<p class="mt-6 text-sm text-slate-500">Кто входит?</p>
			<div class="mt-3 space-y-2">
				{#each admins as admin}
					<button
						type="button"
						onclick={() => (userId = admin.id)}
						class="h-12 w-full rounded-md border text-base font-semibold {userId === admin.id

...
Источники требований: [`idea.md`](idea.md), [`idea_waiter.md`](idea_waiter.md), [`idea_kitchen.md`](idea_kitchen.md), [`idea_admin.md`](idea_admin.md), [`plans/ARCHITECTURE.md`](plans/ARCHITECTURE.md), [`plans/WAITER_UX.md`](plans/WAITER_UX.md).
...
- PWA: `static/manifest.webmanifest` + `src/service-worker.ts`. **Не** кэшировать `/api/*` и SSE. Service worker не должен отдавать устаревшее меню как истину — меню обновляется событием `MENU_UPDATED`.
...
Официант — **главный** интерфейс. Не сводить конструктор пречека к «список + форма». Обязательны (см. `plans/WAITER_UX.md`):

- плиточное меню (2 колонки на телефоне);
- красный балун количества, long-press → `−`/`+`;
- шторка заказа: peek + оттягивание **вниз**, snap peek / half / full;
- гости и сплит (tap-to-move, не только DnD);
- «На кухню» (`held` → `pending`); официант не удаляет уже отправленное;
- оплата по гостям; пречек `closed` только когда все гости оплачены;
- отмена пречека — только админ.

Кухня: крупные зоны (≥ 56px), FIFO, таймер, «нет блюда» с подтверждением.
